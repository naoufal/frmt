...
var frmt = {
  ...
  dateOfBirth: function(value) {
    ...
    if ((value.length === 3) || (value.length === 4)) {
      f_value = value.match(/.{1,2}/g);

      if (value.length === 3) {
        // if second value is bigger than 1, add a zero in front of it
        if (parseInt(f_value[1]) > 1) {
          f_value[1] = '0' + f_value[1];
        }
      }

      f_value = f_value.join(' / ');
    }

    if (value.length > 4) {
      f_value = value.match(/(\d{1,2})(\d{1,2})?(\d{1,4})?/).join(' / ');
      f_value = f_value.substr(f_value.indexOf(' ') + 3);
    }

    return f_value;
  },

  dateOfBirthToDate: function(value) {
    if (!value) {
      value = [];
    }
